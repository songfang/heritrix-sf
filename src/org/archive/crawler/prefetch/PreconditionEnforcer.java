...
/**
 * Ensures the preconditions for a fetch -- such as DNS lookup or acquiring and
 * respecting a robots.txt policy -- are satisfied before a URI is passed to
 * subsequent stages.
 *
 * @author gojomo
 */
public class PreconditionEnforcer extends Processor implements
		CoreAttributeConstants, FetchStatusCodes
{
	...
	private static final Logger logger = Logger
			.getLogger(PreconditionEnforcer.class.getName());

	private final static Integer DEFAULT_IP_VALIDITY_DURATION = new Integer(
			60 * 60 * 6); // six hours
	private final static Integer DEFAULT_ROBOTS_VALIDITY_DURATION = new Integer(
			60 * 60 * 24); // one day

	/** seconds to keep IP information for */
	public final static String ATTR_IP_VALIDITY_DURATION = "ip-validity-duration-seconds";
	/** seconds to cache robots info */
	public final static String ATTR_ROBOTS_VALIDITY_DURATION = "robot-validity-duration-seconds";
	...
	public final static String ATTR_CALCULATE_ROBOTS_ONLY = "calculate-robots-only";
	 ...
	 *
	 * @param curi
	 *            CrawlURI we're checking for any required preconditions.
	 * @return True, if this <code>curi</code> has a precondition or processing
	 *         should be terminated for some other reason. False if we can
	 *         precede to process this url.
	 */
	private boolean considerRobotsPreconditions(CrawlURI curi)
	{
		/*
		 * // treat /robots.txt fetches specially UURI uuri = curi.getUURI();
		 * try { if (uuri != null && uuri.getPath() != null &&
		 * curi.getUURI().getPath().equals("/robots.txt")) { // allow processing
		 * to continue curi.setPrerequisite(true); return false; } } catch
		 * (URIException e) { logger.severe("Failed get of path for " + curi); }
		 * // require /robots.txt if not present if (isRobotsExpired(curi)) { //
		 * Need to get robots if (logger.isLoggable(Level.FINE)) {
		 * logger.fine("No valid robots for " +
		 * getController().getServerCache().getServerFor(curi) + "; deferring "
		 * + curi); }
		 * 
		 * // Robots expired - should be refetched even though its already //
		 * crawled. try { String prereq = curi.getUURI().resolve("/robots.txt")
		 * .toString(); curi.markPrerequisite(prereq, getController()
		 * .getPostprocessorChain()); } catch (URIException e1) {
		 * logger.severe("Failed resolve using " + curi); throw new
		 * RuntimeException(e1); // shouldn't ever happen } return true; } //
		 * test against robots.txt if available CrawlServer cs =
		 * getController().getServerCache().getServerFor(curi); if
		 * (cs.isValidRobots()) { String ua =
		 * getController().getOrder().getUserAgent(curi); if
		 * (cs.getRobots().disallows(curi, ua)) { if (((Boolean)
		 * getUncheckedAttribute(curi,
		 * ATTR_CALCULATE_ROBOTS_ONLY)).booleanValue() == true) { // annotate
		 * URI as excluded, but continue to process // normally
		 * curi.addAnnotation("robotExcluded"); return false; } // mark as
		 * precluded; in FetchHTTP, this will // prevent fetching and cause a
		 * skip to the end // of processing (unless an intervening processor //
		 * overrules) curi.setFetchStatus(S_ROBOTS_PRECLUDED);
		 * curi.putString("error", "robots.txt exclusion");
		 * logger.fine("robots.txt precluded " + curi); return true; } return
		 * false; } // No valid robots found => Attempt to get robots.txt failed
		 * curi.skipToProcessorChain(getController().getPostprocessorChain());
		 * curi.setFetchStatus(S_ROBOTS_PREREQUISITE_FAILURE);
		 * curi.putString("error", "robots.txt prerequisite failed"); if
	...
	/**
	 * @param curi
	 *            CrawlURI whose dns prerequisite we're to check.
	...
	private boolean considerDnsPreconditions(CrawlURI curi)
	{
		...
		if (ch == null || ch.hasBeenLookedUp() && ch.getIP() == null)
		{
			if (logger.isLoggable(Level.FINE))
			{
				logger.fine("no dns for " + ch
						+ " cancelling processing for CrawlURI "
						+ curi.toString());
			}
		...
		if (isIpExpired(curi) && !curi.getUURI().getScheme().equals("dns"))
		{
			...
			try
			{
				curi.markPrerequisite(preq, getController()
	 ...
	 *
	 * @param curi
	 *            the uri this time is valid for.
		...
		while (i.hasNext())
		{
			...
			if (!authenticated(c, curi))
			{
				...
				} else
				{
					try
					{
						curi.markPrerequisite(prereq, getController()
								.getPostprocessorChain());
					} catch (URIException e)
					{
						logger.severe("unable to set credentials prerequisite "
								+ prereq);
			...
			final CrawlURI curi)
	{
		...
		for (Iterator i = avatars.iterator(); i.hasNext();)
		{
			...
			{
				logger.severe("Failed getting key for " + credential + " for "
						+ curi);
				continue;
			}
